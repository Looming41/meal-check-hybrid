 ...
 */
import { DISEASE_BY_ID, DISEASE_GROUPS, evaluate, sumMeal, verdictText, resolveProfile } from './rules.js';
...
// aistudio.google.com에서 발급받은 키를 여기 붙여넣으면 곧바로 Gemini 인식이 켜집니다.
// 실사용자는 이 값을 안 쓰고 전부 설정 화면에서 본인 키를 붙여넣는다(BYOK) —
// Gemini는 프록시 없이 항상 사용자 본인 키로 구글에 직접 붙는다(2026-08-22).
//
...
//
// ⚠️ 보안: 이 키가 든 파일을 인터넷에 올리면 누구나 훔쳐 씁니다. 품질만 확인하고 반드시 지우세요.
const DEV_GEMINI_KEY = '';

// 쓸 Gemini 모델.
// 2026-08-21에 -latest 별칭으로 바꿨다가 "Failed to fetch"가 재현돼 구체적
// 버전(gemini-3.7-flash)으로 되돌린 적이 있고, 2026-08-22 직접 비교 curl
...
const store = new NutritionStore({ profile: MFDS_PROFILE });
...
function geminiReady() { return !!(DEV_GEMINI_KEY || safeStore.get('mc.gemini_key')); }
...
function pickDefaultAdapter() {
  return geminiReady() ? 'gemini' : LOCAL_READY ? 'local' : 'ondevice';
}
...
function renderUsage() {
  ...
  el.textContent = tf('usageLocalLine', { count, limit: GEMINI_DAILY_LIMIT, left })
    + (left <= 20 ? t('usageResetNote') : '');
...
for (const g of DISEASE_GROUPS) {
  ...
  for (const id of g.ids) {
    ...
    /* I18N 상수가 이 시점(모듈 최상위 실행)엔 아직 초기화되지 않아 t()를 여기서
       바로 못 쓴다 — 일단 한국어로 채우고, applyI18n()이 나중에 다시 채운다. */
    b.dataset.focus = d.focus;
...
function updateProfileSummary() {
  ...
  if (p.ageYears) parts.push(tf('profileSummaryAge', { a: p.ageYears }));
  if (p.sex) parts.push(p.sex === 'f' ? t('sexF') : t('sexM'));
  const tag = $('profileBox').querySelector('.opt-tag');
  tag.textContent = parts.length ? parts.join(' · ') : t('optTag');
}
...
function renderAdapterOptions() {
  ...
  for (const A of ADAPTERS) {
    ...
    b.textContent = t(`adapterLabel_${A.id}`) + (usable ? '' : t('adapterNeedsSetup'));
    const small = document.createElement('small');
    small.textContent = t(`adapterBlurb_${A.id}`);
    label.append(input, b, small);

    /* 못 쓰는 이유만 적어 두면 사용자는 거기서 막힌다. 켜는 방법을 함께 적는다. */
    const HOWTO = { gemini: t('howtoGemini'), local: t('howtoLocal') };
    if (!usable && HOWTO[A.id]) {
      const how = document.createElement('small');
      how.className = 'howto';
      how.textContent = t('howToEnablePrefix') + HOWTO[A.id];
...
function applyAdapterMode() {
  ...
  $('step2Title').textContent = t(manual ? 'step2TitleManual' : 'step2TitleAuto');
  $('step2Hint').textContent = t(manual ? 'step2HintManual' : 'step2HintAuto');
  renderUsage();
  syncButtons();
}
...
$('scanBtn').addEventListener('click', async () => {
  hideErr();
  /* 여기 opts의 model은 어댑터마다 뜻이 다르다(Gemini는 모델 ID, 온디바이스는
     onDeviceModel이라는 별도 키를 쓴다) — makeIdentifier()가 어댑터별로
     정확히 필요한 필드만 골라 각 생성자에 넘기므로 서로 안 섞인다.
     Gemini 전용 필드(devApiKey)는 사용자 본인 키가 있으면 그걸 우선하도록
     geminiIdentifierOpts()를 맨 뒤에 펼쳐 기본값을 덮어쓰게 한다. */
  ...
  if (adapterId === 'gemini') bumpUsage();
  setLoading(true, t(adapterId === 'ondevice' ? 'loadingScanOndevice' : 'loadingScan'));
  ...
  try {
    ...
    if (adapterId === 'ondevice' || adapterId === 'local') {
      ...
      await ident.prepare(p => {
        if (p.done) { setLoadingText(t('loadingAnalyzing')); setProgress(null); }
        else { setLoadingText(tf('loadingModelDownload', { pct: p.pct })); setProgress(p.pct); }
      });
    }
    ...
    if (unsure && adapterId === 'local' && geminiReady()) {
      setLoadingText(t('loadingDoubleCheck'));
      try {
    ...
    if (found.unmatched?.length) {
      showErr(tf('errUnmatched', { names: found.unmatched.map(u => u.name).join(', ') }));
    } else if (found.length === 0) {
      showErr(t('errNoFoodFound'));
    } else if (unsure) {
      showErr(t('errUnsure'));
    ...
    const detail = tf('errDetailSuffix', { msg: e.message });

    if (blocked && IS_FILE) {
      showErr(t('errBlockedFile') + detail);
    } else if (blocked) {
      showErr(t('errBlockedGeneric') + detail);
    } else if (e.message === 'RATE_LIMIT') {
      /* Gemini 429는 분당 한도와 하루 한도 두 가지가 있다.
         어느 쪽인지에 따라 기다리는 시간이 완전히 다르므로 구분해 안내한다. */
      const { count } = readUsage();
      showErr((count >= GEMINI_DAILY_LIMIT
        ? tf('errRateLimitDaily', { limit: GEMINI_DAILY_LIMIT })
        : t('errRateLimitMinute')) +
        t('hintPickNow') + detail);
      ...
      showErr(t('errBadKey') + t('hintPickNow') + detail);
    } else if (e.message === 'SERVER_ERROR') {
      showErr(t('errServerError') + t('hintPickNow') + detail);
    } else if (e.message === 'PARSE' || e.message === 'EMPTY') {
      showErr(t('errParseEmpty') + detail);
    } else if (e.message === 'NETWORK') {
      showErr(t('errNetwork') + t('hintPickSameEven') + detail);
    } else {
      showErr(t('errGenericFallback') + t('hintPickSameEven') + detail);
    }
...
$('foodSearch').addEventListener('input', async e => {
  ...
  /* 내장에서 충분히 나오면 원격은 부르지 않는다. 호출을 아끼는 것이 목적이다.
     식약처 원격 검색은 이제 프록시 없이 항상 시도한다(store.searchRemote()가
     키가 없을 때는 알아서 빈 배열을 준다). */
  if (local.length >= 3 || q.trim().length < 2) return;
...
/* ── 고른 음식 목록 ──────────────────────────────────────── */
const PORTIONS = [[0.25, 'portion14'], [0.5, 'portionHalf'], [0.75, 'portion34'],
                  [1, 'portion1'], [1.5, 'portion15'], [2, 'portion2']];
...
function renderAlternatives(alts, pickMode) {
  ...
  lab.textContent = t(pickMode ? 'altPickLabel' : 'altSwapLabel');
  box.appendChild(lab);
...
function renderPicked() {
  ...
  for (const item of picked) {
    ...
    if (item.confidence < 0.999) {
      ...
      tag.textContent = lvl === 'hi' ? t('confHigh') : lvl === 'mid' ? t('confMid') : t('confLow');
    ...
    if (item.food.src === 'gemini_est') {
      ...
      tag.className = 'src-tag';
      tag.style.marginLeft = '8px';
      tag.textContent = t('aiEstimateTag');
      nm.appendChild(tag);
    }
    const sub = document.createElement('small');
    sub.textContent = tf('servingGrams', { g: item.food.serving_g });
    nm.appendChild(sub);

    const sel = document.createElement('select');
    sel.setAttribute('aria-label', tf('portionAriaLabel', { name: item.food.ko }));
    for (const [v, key] of PORTIONS) {
    ...
    del.className = 'del';
    del.textContent = t('deleteX');
    del.setAttribute('aria-label', tf('deleteAriaLabel', { name: item.food.ko }));
...
$('judgeBtn').addEventListener('click', async () => {
  ...
  /* 담긴 음식 중 하나라도 우리 DB·식약처에 없어 Gemini의 추정치("가상의 음식",
     id가 gemini_est:로 시작)로 대신 채워진 게 있으면, 규칙 엔진이 애초에
     부정확한 수치로 판정하게 된다 — 그럴 땐 사진+질환을 다시 Gemini에게 보내
     직접 판정까지 받는 게 낫다. 반대로 담긴 음식이 전부 DB·식약처에 실제로
     있는 것이면 규칙 엔진만으로 이미 정확하게 판정되므로, Gemini를 또 부르는
     건 순전히 API 호출 낭비다(무료 등급 분당 한도를 사진 식별과 나눠 써야
     하는데 필요 없이 반씩 깎아먹는다). 그래서 이 조건이 필요할 때만 켜진다. */
  const needsGeminiJudge = picked.some(p => p.food.id?.startsWith('gemini_est:'));
  if (imageDataUrl && adapterId === 'gemini' && geminiReady() && diseases.length && needsGeminiJudge) {
    setLoading(true, t('loadingAiJudging'));
    try {
      const g = geminiIdentifierOpts();
      const parsed = await judgeMealWithGemini(imageDataUrl, diseases, profile,
        { devApiKey: g.devApiKey, model: g.model });
      bumpUsage();
      ...
      const reason =
        ...
        : networkIssue
          ? t('reasonNetwork')
          : e.message;
      showErr(tf('errAiJudgeFallback', { reason }) + tf('errDetailSuffix', { msg: e.message }));
      /* setLoading(false)는 아래 규칙 엔진 경로가 다시 setLoading(true)를 부르므로 생략 */
    }
  }

  setLoading(true, t('loadingNutrition'));
  try {
    await store.ready();
    ...
    render(evaluate(meal, ids, readProfile(), currentLang()), resolved, meal);
  } catch (e) {
    showErr(tf('errJudgeGeneric', { msg: e.message }));
  } finally {
    setLoading(false);
  }
});
...
function render(res, resolved, meal) {
  ...
  for (const c of res.conflicts) {
    ...
    head.textContent = t('conflictHeading');
  ...
  for (const n of res.notes) {
    ...
    d.className = 'note';
    const head = document.createElement('b');
    head.textContent = tf('noteHeadingTemplate', { disease: diseaseDisplayName(n.disease) });
    const p = document.createElement('span');
    p.textContent = n.msg;
    d.append(head, p);
    nb.appendChild(d);
  ...
  if (res.aiJudged) {
    ...
    d.className = 'note';
    const head = document.createElement('b');
    head.textContent = t('aiJudgedHeading');
    const p = document.createElement('span');
    p.textContent = t('aiJudgedExplain');
    d.append(head, p);
    nb.appendChild(d);
  }
  ...
  for (const d of res.byDisease) {
    ...
    nm.textContent = diseaseDisplayName(d.disease);
    const badge = document.createElement('span');
    badge.className = 'badge';
    badge.textContent = verdictText(d.verdict, currentLang());
    row.append(nm, badge);
    ...
    if (d.triggers.length) {
      ...
      sm.textContent = t('evidenceSummary');
      const why = document.createElement('div');
      why.className = 'why';
      for (const trig of d.triggers) {
        const line = document.createElement('div');
        if (trig.kind === 'numeric') {
          const code = document.createElement('code');
          code.textContent = `${trig.label} ${trig.value}${trig.unit}`;
          line.append(code, document.createTextNode(
            ` — ${verdictText(trig.level, currentLang())}` + tf('thresholdSuffix', { threshold: trig.threshold, unit: trig.unit })));
          // 프로필로 임계값이 조정됐으면 무엇 때문인지 밝힌다
          if (trig.scaledBy) {
            const s = document.createElement('span');
            s.style.cssText = 'color:var(--blue);font-weight:700';
            s.textContent = tf('scaledBySuffix', { scaledBy: trig.scaledBy });
            line.appendChild(s);
          }
        } else {
          line.textContent = `· ${trig.text}`;
        }
        const rat = document.createElement('div');
        rat.style.cssText = 'font-size:12px;opacity:.75;margin-top:2px';
        rat.textContent = trig.rationale.replace(/^RATIONALE:\s*/, t('rationaleLabel'));
  ...
  /* 식약처에서 온 음식은 퓨린·요오드 등이 비어 있다. 그 축을 쓰는 질환을
     ...
     매칭은 항상 한국어 이름(res.byDisease가 rules.js에서 그대로 오므로)으로 하고,
     화면에 보여줄 때만 AXIS_DISEASE_KEY로 다국어 이름을 찾는다. */
  const AXIS_DISEASE = {
    purine_mg: '통풍', vitk_ug: '와파린', iodine_ug: '갑상선',
    iron_mg: '빈혈', caffeine_mg: '빈혈·임신', vita_ug: '임신'
  };
  const AXIS_DISEASE_KEY = {
    통풍: 'axisGout', 와파린: 'axisWarfarin', 갑상선: 'axisThyroid',
    빈혈: 'axisAnemia', '빈혈·임신': 'axisAnemiaPreg', 임신: 'axisPregnancy'
  };
  ...
  if (!res.aiJudged && remoteFoods.length) {
    ...
    d.className = 'note';
    const head = document.createElement('b');
    head.textContent = t('incompleteNutritionHeading');
    const txt = document.createElement('span');
    txt.textContent =
      tf('incompleteNutritionBody', { foods: remoteFoods.map(r => r.food.ko).join(', ') }) +
      (relevant.length
        ? tf('incompleteNutritionRelevant', { diseases: relevant.map(name => t(AXIS_DISEASE_KEY[name]) || name).join('·') })
        : t('incompleteNutritionIrrelevant'));
    d.append(head, txt);
    nb.appendChild(d);
  }
  ...
  const lines = res.aiJudged ? [] : (() => {
    ...
    const profLine = (p.weightGiven || p.ageYears)
      ? tf('profLineAdjusted', { parts: [p.weightGiven ? tf('profWeightPart', { w: p.weightKg }) : null,
                       p.ageYears ? tf('profAgePart', { a: p.ageYears }) : null].filter(Boolean).join(' · ') })
      : t('profLineDefault');
    return [
      profLine,
      pv.total === 0 ? '' :
        pv.pct > 0
          ? tf('pvPctLine', { pct: pv.pct })
          : t('pvAllBuiltin'),
      t('pvLowAccuracyNote'),
      tf('mealTotalsLine', {
        kcal: Math.round(meal.kcal), sodium: Math.round(meal.sodium_mg),
        potassium: Math.round(meal.potassium_mg), phosphorus: Math.round(meal.phosphorus_mg),
        sugar: Math.round(meal.sugar_g), satfat: Math.round(meal.satfat_g * 10) / 10,
        iodine: Math.round(meal.iodine_ug), iron: Math.round(meal.iron_mg * 10) / 10
      })
    ].filter(Boolean);
  })();
...
function showOnlySettingsSection(id) {
  for (const s of document.querySelectorAll('.settings-section')) {
    s.hidden = (s.id !== id);
    if (s.tagName === 'DETAILS' && s.id === id) s.open = true;
  }
}
/** 평소 설정 화면: 아코디언이므로 기본은 전부 접어 두고, 눌러야 펼쳐지게 한다. */
function showAllSettingsSections() {
  for (const s of document.querySelectorAll('.settings-section')) {
    s.hidden = false;
    if (s.tagName === 'DETAILS') s.open = false;
  }
}
/** 온보딩 중에는 지금 단계의 아코디언을 사용자가 실수로 접어도 다시 펴 준다
    (안 그러면 입력칸이 사라져 다음으로 못 넘어가는 것처럼 보인다). */
for (const d of document.querySelectorAll('details.settings-section')) {
  d.addEventListener('toggle', () => {
    if (!d.open && !d.hidden && !$('wizardNextBtn').hidden) d.open = true;
  });
}
...
const I18N = {
  ko: {
    ...
    profileClear: '입력값 지우기', profileSummaryAge: '{a}세',
    pfWeightPlaceholder: '예: 62', pfAgePlaceholder: '예: 34',
    ...
    // ── 헤더 · 아리아 라벨 ──
    ariaSettings: '설정', ariaSettingsClose: '설정 닫기', ariaLangSelect: '언어 선택',
    ariaDiseaseSelect: '질환 및 상태 선택', ariaPhotoZone: '사진 찍기 또는 올리기', ariaFoodSearch: '음식 검색',
    focusTooltipPrefix: '주의 항목: {focus}',

    // ── 사용량 카운터 ──
    usageLocalLine: '오늘 무료 한도 {count}/{limit}회 사용 · {left}회 남음 (기기 기준 추정)',
    usageResetNote: ' — 다 쓰면 내일 자정(태평양시)에 초기화됩니다.',

    // ── 2단계: 사진 ──
    photoZoneTitle: '사진 찍기 · 올리기', photoZoneHint: '여기를 누르면 카메라나 앨범이 열립니다',
    previewAlt: '선택한 음식 사진 미리보기',
    step2TitleAuto: '음식 사진 찍기', step2TitleManual: '사진 올리기 (선택)',
    step2HintAuto: '밥상 전체가 위에서 잘 보이게, 밝은 곳에서 찍을수록 정확합니다.',
    step2HintManual: '사진은 참고용입니다. 보면서 아래에서 음식을 골라 담으세요.',
    scanBtnLabel: '사진에서 음식 찾기',

    // ── 로딩 문구 ──
    loadingScanOndevice: '기기에서 음식을 살펴보고 있습니다…',
    loadingScan: '음식을 살펴보고 있습니다…',
    loadingAnalyzing: '사진을 분석하고 있습니다…',
    loadingModelDownload: '처음 한 번만 모델을 내려받습니다… {pct}%',
    loadingDoubleCheck: '확신이 낮아 온라인으로 한 번 더 확인하고 있습니다…',
    loadingAiJudging: 'AI가 사진을 보고 직접 판정하고 있습니다…',
    loadingNutrition: '영양 정보를 확인하고 있습니다…',

    // ── 인식 오류 안내(스캔) ──
    errUnmatched: '"{names}"은(는) 영양 정보가 없어 목록에 넣지 못했습니다. 아래에서 비슷한 음식을 직접 찾아 더해 주세요. 빠뜨리면 판정이 실제보다 관대해집니다.',
    errNoFoodFound: '사진에서 아는 음식을 찾지 못했습니다. 아래에서 직접 골라 주세요.',
    errUnsure: '사진 속 음식을 확실하게 알아보지 못했습니다. 아래 후보 중에 있으면 눌러 주시고, 없으면 검색해서 직접 골라 주세요.',
    errSinglePick: '사진에서 음식 하나만 알아냈습니다. 밥·국·반찬이 더 있으면 아래에서 직접 더해 주세요. 빠뜨리면 판정이 실제보다 관대해집니다.',
    errBlockedFile: '이 브라우저는 파일을 직접 열었을 때 AI 모델 불러오기를 막습니다. 사진은 그대로 두고 아래에서 음식을 검색해 담으시면 판정은 똑같이 됩니다. 자동 인식을 쓰시려면 같은 폴더의 열기.command로 열어 주세요.',
    errBlockedGeneric: 'AI에 연결하지 못했습니다. 인터넷 연결을 확인하고 다시 시도해 주세요. 아래에서 직접 골라 주셔도 판정은 똑같이 됩니다.',
    errRateLimitDaily: '오늘 무료 한도({limit}회)를 다 썼습니다. 내일 자정(태평양시)에 초기화됩니다. ',
    errRateLimitMinute: '요청이 잠깐 몰렸습니다. 1분쯤 뒤에 다시 눌러 주세요. ',
    hintPickNow: '아래에서 직접 골라 주시면 지금 바로 판정할 수 있습니다.',
    hintPickSameEven: '아래에서 직접 골라 주셔도 판정은 똑같이 됩니다.',
    errBadKey: '저장된 Gemini API 키가 거부됐습니다. 키가 삭제됐거나 무료 한도를 다 썼을 수 있습니다. ☰ 설정 → Gemini API 키에서 다시 확인하거나 새 키를 붙여넣어 주세요. ',
    errServerError: '구글 서버가 지금 일시적으로 불안정합니다(저희 앱 문제가 아닙니다). 잠시 후 다시 시도해 주세요. ',
    errParseEmpty: 'AI 응답을 읽지 못했습니다. 다시 시도하시거나 아래에서 직접 골라 주세요.',
    errNetwork: '인터넷 연결을 확인해 주세요. ',
    errGenericFallback: '음식을 찾지 못했습니다. ',
    errDetailSuffix: ' [상세: {msg}]',

    // ── 판정 버튼: AI 직접 판정 실패 → 규칙 엔진 폴백 ──
    reasonQuotaOrRate: '오늘 무료 한도를 다 썼거나 요청이 몰렸습니다',
    reasonBadKey: '저장된 키가 거부됐습니다 — ☰ 설정에서 Gemini 키를 다시 확인해 주세요',
    reasonServerError: '구글 서버가 지금 일시적으로 불안정합니다(저희 앱 문제 아님) — 잠시 후 다시 시도해 주세요',
    reasonNetwork: '인터넷 연결이 불안정합니다',
    errAiJudgeFallback: 'AI 직접 판정에 실패해 자체 규칙으로 대신 판정했습니다. ({reason})',
    errJudgeGeneric: '판정 중 문제가 생겼습니다. ({msg})',

    // ── 음식 목록 ──
    recentLabel: '최근 먹은 것', recentClear: '지우기',
    foodSearchPlaceholder: '음식 이름을 넣어 찾기 (예: 된장찌개)',
    servingGrams: '1인분 {g}g',
    confHigh: '확실', confMid: '아마도', confLow: '확인 필요',
    aiEstimateTag: 'AI 추정', mfdsTag: '식약처',
    portion14: '4분의 1', portionHalf: '반', portion34: '4분의 3',
    portion1: '1인분', portion15: '1.5인분', portion2: '2인분',
    portionAriaLabel: '{name} 양', deleteAriaLabel: '{name} 지우기', deleteX: '✕',
    altPickLabel: '이 중에 있나요? (누르면 목록에 담습니다)',
    altSwapLabel: '혹시 이건가요? (누르면 바꿔 담습니다)',
    confirmHint: 'AI가 틀릴 수 있습니다. 빠진 것은 더하고, 잘못된 것은 지워 주세요. 양도 고칠 수 있습니다.',
    emptyNote: '아직 고른 음식이 없습니다. 위에서 검색해 더해 주세요.',

    // ── 결과 화면 ──
    stepTagResult: '판정 결과',
    mealLabel: '이 식사: ',
    conflictHeading: '⚖️ 고른 항목끼리 권고가 엇갈립니다',
    noteHeadingTemplate: 'ℹ️ {disease} — 이 판정이 다루지 않는 것',
    aiJudgedHeading: 'ℹ️ 이 판정은 AI가 사진을 보고 직접 냈습니다',
    aiJudgedExplain: '자체 데이터베이스에 없는 음식이라 규칙 엔진 대신 Gemini가 판정했습니다. 같은 사진이라도 다시 판정하면 표현이 달라질 수 있고, 항목별 수치 근거는 보여드릴 수 없습니다.',
    evidenceSummary: '이 판정의 근거 보기',
    rationaleLabel: '근거: ',
    thresholdSuffix: ' 기준 {threshold}{unit} 이상',
    scaledBySuffix: ' ({scaledBy} 기준으로 조정됨)',
    incompleteNutritionHeading: '⚠️ 일부 음식은 영양 정보가 불완전합니다',
    incompleteNutritionBody: '{foods}은(는) 식약처 DB에서 가져왔습니다. 식약처는 퓨린·비타민K·요오드·철·카페인을 제공하지 않아 이 값들이 0으로 계산됐습니다. ',
    incompleteNutritionRelevant: '고르신 {diseases} 판정이 실제보다 관대하게 나왔을 수 있습니다.',
    incompleteNutritionIrrelevant: '해당 축을 쓰는 질환을 고르지 않아 이번 판정에는 영향이 적습니다.',
    profLineAdjusted: '기준 조정: {parts}',
    profWeightPart: '체중 {w}kg', profAgePart: '나이 {a}세',
    profLineDefault: '체중·나이를 넣지 않아 성인 60kg 기준으로 판정했습니다.',
    pvPctLine: '영양 수치의 {pct}%는 식약처 식품영양성분DB, 나머지는 내장 추정치입니다.',
    pvAllBuiltin: '영양 수치는 모두 내장 추정치입니다. (식약처 API 미연결)',
    pvLowAccuracyNote: '퓨린·비타민K·요오드·철·카페인·레티놀은 식약처 DB에 없어 문헌 추정치를 씁니다. 통풍·와파린·갑상선·빈혈·임신 판정은 다른 항목보다 정확도가 낮습니다.',
    mealTotalsLine: '열량 {kcal}kcal · 나트륨 {sodium}mg · 칼륨 {potassium}mg · 인 {phosphorus}mg · 당류 {sugar}g · 포화지방 {satfat}g · 요오드 {iodine}µg · 철 {iron}mg',
    axisGout: '통풍', axisWarfarin: '와파린', axisThyroid: '갑상선',
    axisAnemia: '빈혈', axisAnemiaPreg: '빈혈·임신', axisPregnancy: '임신',
    tipsHeading: '이렇게 드시면 더 좋아요', retryBtn: '다른 음식 판정하기',
    disclaimerText: '이 판정은 일반적인 식이 원칙을 규칙으로 옮긴 참고 정보이며, 의료 진단이나 처방이 아닙니다. 정확한 식사 계획은 담당 의사·영양사와 상의하세요. 음식 인식과 양 추정에는 오차가 있습니다. 임신 중이거나 성장기 아동의 식사는 특히 임의로 줄이지 마시고 전문가와 상의하세요.',

    // ── 인식 방법 바꾸기 (설정) ──
    recognitionMethodSummary: '⚙️ 음식 인식 방법 바꾸기',
    recognitionMethodNote: '어떤 방법을 쓰든 판정 기준은 똑같습니다. 인식 방법만 달라집니다.',
    adapterNeedsSetup: ' (설정 필요)',
    howToEnablePrefix: '켜는 법: ',
    howtoGemini: 'aistudio.google.com에서 키를 받아 app.js의 DEV_GEMINI_KEY에 붙여넣고 npm run build. 무료 등급은 신용카드가 필요 없고, 결제를 켜지 않으면 요금이 청구되지 않습니다.',
    howtoLocal: 'train/ 폴더의 안내대로 한국 음식 데이터로 모델을 학습한 뒤, 내보낸 폴더 주소를 app.js의 LOCAL_MODEL_URL에 넣으세요. 학습만 되면 호출 비용도 한도도 없습니다.',
    adapterLabel_gemini: 'AI로 인식 (정확도 높음)', adapterBlurb_gemini: '가장 정확합니다. 사진이 서버로 전송되며 하루 사용 한도가 있습니다.',
    adapterLabel_local: '자체 모델로 인식 (한국 음식 특화)',
    adapterBlurb_local: '우리가 한국 음식으로 직접 학습시킨 모델입니다. 기기에서 돌아 사진이 밖으로 나가지 않고, 호출 비용도 한도도 없습니다.',
    adapterLabel_ondevice: '기기에서 인식',
    adapterBlurb_ondevice: '사진이 기기 밖으로 나가지 않습니다. 인터넷·키 불필요. 처음 한 번 모델을 내려받습니다(약 150MB).',
    adapterLabel_manual: '직접 고르기', adapterBlurb_manual: 'AI를 쓰지 않습니다. 정확도가 가장 높고 인터넷 없이도 됩니다.',

    // ── 첫 실행 면책 고지 ──
    gateTitle: '먼저 읽어 주세요',
    gateItem1Title: '의료기기가 아닙니다',
    gateItem1Body: '일반적인 식이 지침을 규칙으로 옮긴 참고 도구입니다. 진단·처방·치료를 대신하지 않습니다. 약 복용, 식사 계획 변경은 반드시 담당 의사·영양사와 상의하세요.',
    gateItem2Title: '영양 수치는 검증되지 않은 추정치입니다',
    gateItem2Body: '수록된 음식 대부분의 영양값이 일반 조리법 기준 추정치이며 아직 영양사 검토를 거치지 않았습니다. 같은 이름의 음식이라도 조리법·업체에 따라 나트륨이 두 배까지 차이 납니다.',
    gateItem2Bold: '판정이 실제와 다를 수 있습니다.',
    gateItem3Title: '일부 항목은 정확도가 특히 낮습니다',
    gateItem3Body: '퓨린·비타민K·요오드·철·카페인·레티놀은 공공 데이터베이스에 없어 문헌 추정치를 씁니다. 통풍·와파린·갑상선·빈혈·임신 판정은 다른 항목보다 오차가 큽니다.',
    gateItem4Title: '응급 상황에는 쓰지 마세요',
    gateItem4Body: '알레르기 반응, 저혈당, 호흡곤란, 삼킴 사고가 의심되면 이 앱을 보지 말고 119에 연락하세요.',
    gateAgree: '위 내용을 이해했으며, 이 앱의 판정을 의료적 판단의 근거로 삼지 않겠습니다.',
    gateBtn: '이해했습니다', gateFootLink: '개인정보 처리방침'
  },
  en: {
    ...
    profileClear: 'Clear', profileSummaryAge: '{a}y',
    pfWeightPlaceholder: 'e.g. 62', pfAgePlaceholder: 'e.g. 34',
    ...
    // ── Header · aria labels ──
    ariaSettings: 'Settings', ariaSettingsClose: 'Close settings', ariaLangSelect: 'Language selection',
    ariaDiseaseSelect: 'Condition selection', ariaPhotoZone: 'Take or upload a photo', ariaFoodSearch: 'Search food',
    focusTooltipPrefix: 'Watch: {focus}',

    // ── Usage counter ──
    usageLocalLine: 'Free daily limit: {count}/{limit} used today · {left} left (device estimate)',
    usageResetNote: ' — resets at midnight Pacific time once you run out.',

    // ── Step 2: photo ──
    photoZoneTitle: 'Take or upload a photo', photoZoneHint: 'Tap here to open your camera or photo library',
    previewAlt: 'Preview of the selected food photo',
    step2TitleAuto: 'Take a photo of your food', step2TitleManual: 'Add a photo (optional)',
    step2HintAuto: 'For the most accurate result, take it from above in good light so the whole meal is visible.',
    step2HintManual: 'The photo is just for reference — pick your food from the list below while looking at it.',
    scanBtnLabel: 'Find food in photo',

    // ── Loading text ──
    loadingScanOndevice: 'Looking at your food on this device…',
    loadingScan: 'Looking at your food…',
    loadingAnalyzing: 'Analyzing the photo…',
    loadingModelDownload: 'Downloading the model (first time only)… {pct}%',
    loadingDoubleCheck: 'Confidence is low, double-checking online…',
    loadingAiJudging: 'AI is checking the photo and judging it directly…',
    loadingNutrition: 'Checking nutrition info…',

    // ── Scan error messages ──
    errUnmatched: '"{names}" has no nutrition data, so it couldn’t be added to the list. Please search for something similar below and add it yourself — leaving it out makes the verdict look better than it really is.',
    errNoFoodFound: 'Couldn’t recognize any food in the photo. Please pick it manually below.',
    errUnsure: 'Couldn’t confidently recognize the food in the photo. Tap it below if it’s among the candidates, or search and pick it yourself.',
    errSinglePick: 'Only one food was recognized in the photo. If there’s rice, soup, or side dishes too, please add them below yourself — leaving them out makes the verdict look better than it really is.',
    errBlockedFile: 'This browser blocks loading the AI model when the file is opened directly. Leave the photo as-is and search for food below — the verdict will be exactly the same. To use automatic recognition, open the app via 열기.command in the same folder instead.',
    errBlockedGeneric: 'Couldn’t connect to the AI. Please check your internet connection and try again. You can also pick food manually below — the verdict will be exactly the same.',
    errRateLimitDaily: 'You’ve used up today’s free limit ({limit} requests). It resets at midnight Pacific time. ',
    errRateLimitMinute: 'Requests got briefly busy. Please try again in about a minute. ',
    hintPickNow: 'You can pick food manually below to get a verdict right away.',
    hintPickSameEven: 'You can also pick food manually below — the verdict will be exactly the same.',
    errBadKey: 'The saved Gemini API key was rejected. It may have been deleted, or its free quota may be used up. Please check or paste a new key under ☰ Settings → Gemini API key. ',
    errServerError: 'Google’s servers are temporarily unstable right now (not an issue with our app). Please try again shortly. ',
    errParseEmpty: 'Couldn’t read the AI’s response. Please try again or pick food manually below.',
    errNetwork: 'Please check your internet connection. ',
    errGenericFallback: 'Couldn’t recognize the food. ',
    errDetailSuffix: ' [detail: {msg}]',

    // ── Judge button: AI direct-judgment failure → rule-engine fallback ──
    reasonQuotaOrRate: 'today’s free limit is used up or requests got busy',
    reasonBadKey: 'the saved key was rejected — please recheck your Gemini key in ☰ Settings',
    reasonServerError: 'Google’s servers are temporarily unstable right now (not an issue with our app) — please try again shortly',
    reasonNetwork: 'your internet connection is unstable',
    errAiJudgeFallback: 'AI direct judging failed, so the rule engine judged it instead. ({reason})',
    errJudgeGeneric: 'Something went wrong while judging. ({msg})',

    // ── Food list ──
    recentLabel: 'Recently eaten', recentClear: 'Clear',
    foodSearchPlaceholder: 'Type a food name to search (e.g. kimchi stew)',
    servingGrams: 'Serving {g}g',
    confHigh: 'Confident', confMid: 'Likely', confLow: 'Needs check',
    aiEstimateTag: 'AI estimate', mfdsTag: 'MFDS',
    portion14: '1/4', portionHalf: 'Half', portion34: '3/4',
    portion1: '1 serving', portion15: '1.5 servings', portion2: '2 servings',
    portionAriaLabel: '{name} amount', deleteAriaLabel: 'Remove {name}', deleteX: '✕',
    altPickLabel: 'Is it one of these? (tap to add to the list)',
    altSwapLabel: 'Did you mean this? (tap to swap it in)',
    confirmHint: 'AI can be wrong. Add anything missing, remove anything wrong, and adjust the amounts.',
    emptyNote: 'No food added yet. Search above to add some.',

    // ── Result screen ──
    stepTagResult: 'Verdict',
    mealLabel: 'This meal: ',
    conflictHeading: '⚖️ Your picks give conflicting recommendations',
    noteHeadingTemplate: 'ℹ️ {disease} — what this verdict doesn’t cover',
    aiJudgedHeading: 'ℹ️ This verdict was made directly by AI from the photo',
    aiJudgedExplain: 'This food isn’t in our database, so Gemini judged it directly instead of the rule engine. The wording may vary if judged again from the same photo, and per-item numeric evidence can’t be shown for this path.',
    evidenceSummary: 'See the evidence for this verdict',
    rationaleLabel: 'Rationale: ',
    thresholdSuffix: ' threshold {threshold}{unit} or more',
    scaledBySuffix: ' (adjusted based on {scaledBy})',
    incompleteNutritionHeading: '⚠️ Some foods have incomplete nutrition data',
    incompleteNutritionBody: '{foods} came from the MFDS database. MFDS doesn’t provide purine, vitamin K, iodine, iron, or caffeine, so these values were calculated as 0. ',
    incompleteNutritionRelevant: 'Your {diseases} verdict may look more lenient than it really is.',
    incompleteNutritionIrrelevant: 'You didn’t pick a condition that uses those values, so this has little effect on this verdict.',
    profLineAdjusted: 'Thresholds adjusted: {parts}',
    profWeightPart: 'weight {w}kg', profAgePart: 'age {a}',
    profLineDefault: 'No weight or age entered, so thresholds used a 60kg-adult baseline.',
    pvPctLine: '{pct}% of the nutrition values are from the MFDS food nutrition DB; the rest are built-in estimates.',
    pvAllBuiltin: 'All nutrition values are built-in estimates. (MFDS API not connected)',
    pvLowAccuracyNote: 'Purine, vitamin K, iodine, iron, caffeine, and retinol aren’t in the MFDS DB, so literature-based estimates are used. Gout, warfarin, thyroid, anemia, and pregnancy verdicts are less accurate than the others.',
    mealTotalsLine: 'Calories {kcal}kcal · Sodium {sodium}mg · Potassium {potassium}mg · Phosphorus {phosphorus}mg · Sugar {sugar}g · Sat. fat {satfat}g · Iodine {iodine}µg · Iron {iron}mg',
    axisGout: 'Gout', axisWarfarin: 'Warfarin', axisThyroid: 'Thyroid',
    axisAnemia: 'Anemia', axisAnemiaPreg: 'Anemia/Pregnancy', axisPregnancy: 'Pregnancy',
    tipsHeading: 'Tips for eating this better', retryBtn: 'Check another meal',
    disclaimerText: 'This verdict is reference information based on general dietary guidelines, not a medical diagnosis or prescription. For an accurate meal plan, please consult your doctor or dietitian. There is margin for error in food recognition and portion estimation. If you’re pregnant or feeding a growing child, please don’t cut things back on your own — consult a professional.',

    // ── Recognition method chooser (settings) ──
    recognitionMethodSummary: '⚙️ Change food-recognition method',
    recognitionMethodNote: 'The judging criteria are the same no matter which method you use — only the recognition method changes.',
    adapterNeedsSetup: ' (setup needed)',
    howToEnablePrefix: 'How to enable: ',
    howtoGemini: 'Get a key at aistudio.google.com, paste it into DEV_GEMINI_KEY in app.js, then run npm run build. The free tier needs no credit card, and you won’t be charged unless you turn on billing.',
    howtoLocal: 'Train a model on Korean food data following the instructions in the train/ folder, then put the exported folder’s address into LOCAL_MODEL_URL in app.js. Once trained, there’s no call cost or limit.',
    adapterLabel_gemini: 'AI recognition (most accurate)', adapterBlurb_gemini: 'The most accurate. Your photo is sent to a server, and there’s a daily usage limit.',
    adapterLabel_local: 'Custom model (Korean food specialized)',
    adapterBlurb_local: 'A model we trained ourselves on Korean food. It runs on your device so your photo never leaves it, with no call cost or limit.',
    adapterLabel_ondevice: 'Recognize on device',
    adapterBlurb_ondevice: 'Your photo never leaves the device. No internet or key needed. The model downloads once (about 150MB).',
    adapterLabel_manual: 'Pick manually', adapterBlurb_manual: 'No AI is used. The most accurate, and works without internet.',

    // ── First-run disclaimer gate ──
    gateTitle: 'Please read this first',
    gateItem1Title: 'This is not a medical device',
    gateItem1Body: 'This is a reference tool that turns general dietary guidelines into rules. It does not replace diagnosis, prescription, or treatment. Always consult your doctor or dietitian before changing medication or meal plans.',
    gateItem2Title: 'Nutrition values are unverified estimates',
    gateItem2Body: 'Nutrition values for most foods here are estimates based on common recipes and haven’t been reviewed by a dietitian yet. Sodium alone can differ by up to double between two dishes with the same name, depending on recipe and maker.',
    gateItem2Bold: 'The verdict may differ from reality.',
    gateItem3Title: 'Some values are especially low-accuracy',
    gateItem3Body: 'Purine, vitamin K, iodine, iron, caffeine, and retinol aren’t in public databases, so literature-based estimates are used. Gout, warfarin, thyroid, anemia, and pregnancy verdicts have more error than the others.',
    gateItem4Title: 'Do not use this in an emergency',
    gateItem4Body: 'If you suspect an allergic reaction, low blood sugar, difficulty breathing, or a swallowing accident, don’t check this app — call 119 (Korea’s emergency number) immediately.',
    gateAgree: 'I understand the above and won’t use this app’s verdicts as a basis for medical decisions.',
    gateBtn: 'I understand', gateFootLink: 'Privacy Policy'
  }
};
...
/** 결과 화면의 질환 이름(카드 제목·안내문 등)은 rules.js가 늘 한국어로 준다.
    이미 칩에 쓰는 DISEASE_NAME_EN을 그대로 재사용해 표시용으로만 바꾼다 —
    판정 로직·매칭은 항상 한국어 원문 그대로 두고, 화면에 그릴 때만 바꾼다. */
function diseaseDisplayName(koName) {
  if (currentLang() !== 'en') return koName;
  const id = Object.values(DISEASE_BY_ID).find(d => d.name === koName)?.id;
  return (id && DISEASE_NAME_EN[id]) || koName;
}

function currentLang() { return safeStore.get(LANG_KEY) === 'en' ? 'en' : 'ko'; }
function t(key) { return I18N[currentLang()][key] ?? I18N.ko[key] ?? key; }
/** t()에 {이름} 자리표시자를 채워 넣는다. 문장 구조가 언어마다 다를 수 있어
    printf류 위치 인자 대신 이름 있는 자리표시자를 쓴다. */
...
function applyI18n() {
  ...
  /* 텍스트가 아니라 속성(aria-label·placeholder·alt)에 들어가는 문구도 같은 방식으로 다룬다.
     data-i18n은 textContent를 통째로 갈아 끼우므로 속성에는 못 쓴다 — 별도 접두사를 둔다. */
  for (const el of document.querySelectorAll('[data-i18n-aria-label]')) {
    const val = I18N[lang][el.dataset.i18nAriaLabel];
  ...
  for (const el of document.querySelectorAll('[data-i18n-alt]')) {
    const val = I18N[lang][el.dataset.i18nAlt];
  ...
  for (const el of document.querySelectorAll('.chip[data-id]')) {
    ...
    if (el.dataset.focus) el.title = tf('focusTooltipPrefix', { focus: el.dataset.focus });
  }
  ...
  /* 아래는 정적 텍스트가 아니라 데이터에서 매번 새로 만들어지는 조각들이다.
     언어를 바꾼 그 순간에도 바로 반영되도록 다시 그린다. */
  renderAdapterOptions();
  applyAdapterMode();
  renderUsage();
  renderRecent();
...
/* ── Gemini API 키 (BYOK — 사용자 본인 키) ──────────────────
   여기에 키가 있으면 그 키로 구글에 직접 붙는다. 프록시 자체가 이제 앱에
   없으므로(2026-08-22 완전히 제거) 서버 비용도, 사용자의 무료 한도 소진
   걱정도 없다. */
const GEMINI_USER_KEY = 'mc.gemini_key';

function geminiIdentifierOpts() {
  const userKey = safeStore.get(GEMINI_USER_KEY) || '';
  return { devApiKey: userKey || DEV_GEMINI_KEY, model: GEMINI_MODEL };
}
